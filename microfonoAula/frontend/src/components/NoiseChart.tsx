...
function ChartContent({ data, roomName, loading }: { data: HistoryReading[], roomName: string, loading: boolean }) {
    const dbValues = data.map((d) => d.db);
    const avg = dbValues.length > 0 ? dbValues.reduce((a, b) => a + b, 0) / dbValues.length : 0;
                ...
                {loading ? (
                    ...
                    <ResponsiveContainer width="100%" height={350}>
                        <ComposedChart data={data}>
                            ...
                            <ReferenceLine y={50} stroke="#22c55e" strokeDasharray="5 5" strokeOpacity={0.5} />
                            <ReferenceLine y={70} stroke="#ef4444" strokeDasharray="5 5" strokeOpacity={0.5} />

                            <Area type="monotone" dataKey="db" fill="#3b82f6" fillOpacity={0.1} stroke="none" />
                            <Line type="monotone" dataKey="db" stroke="#3b82f6" strokeWidth={2} dot={false} name="Nivel" />
                            <Line type="monotone" dataKey="peak" stroke="#ef4444" strokeWidth={1} strokeDasharray="3 3" dot={false} name="Pico" />
                        </ComposedChart>
                    </ResponsiveContainer>
                )}
            </div>
...
export default function NoiseChart({ data, roomName, onClose, loading, embedded }: NoiseChartProps) {
    // Modo embebido (dentro de la pagina, no modal)
    if (embedded) {
        return (
            <div className="bg-gray-900 border border-gray-800 rounded-2xl">
                <div className="p-6 border-b border-gray-800">
                    <h2 className="text-2xl font-bold text-white">{roomName}</h2>
                    <p className="text-sm text-gray-400 mt-1">Historico de nivel de ruido</p>
                </div>
                <ChartContent data={data} roomName={roomName} loading={loading} />
            </div>
        );
    }
